...
Lemma ftlr_lend {i trans' mem_acc_tx ai regs ps_acc p_tx p_rx ps_na instr trans rx_state r0}:
  ...
  Proof.
    ...
    { (* all pages are exclusively owned, ok to perceed *)
      ...
      iApply ("IH" $! _  _ (ps_acc∖ ps_lend) _ trans'' _ Htotal_regs' with "[] [] [] [] regs tx pgt_tx pgt_acc pgt_acc' LB
      ...
      }
      {
        ...
        assert ((ps_lend ∪ {[p_rx; p_tx]} ∪ (ps_lend ∪ pages_in_trans (trans_memory_in_trans i trans))) =
                  (ps_lend ∪ {[p_rx; p_tx]} ∪ pages_in_trans (trans_memory_in_trans i trans))) as -> by set_solver +.
        iFrame "pgt_owned".
      }
      {
