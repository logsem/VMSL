...
Section logrel_extra.
  ...
  Lemma reg_bigM_split reg i r w:
    reg !! r = Some w ->
    ((full_reg_map reg ∗ [∗ map] k↦y ∈ reg, k @@ i ->r y)%I
     ⊢  (r @@ i ->r w) ∗ (∀ w', r @@ i ->r w' -∗ ∃ reg', (full_reg_map reg' ∗ [∗ map] k↦y ∈ reg', k @@ i ->r y)))%I.
  Proof.
    iIntros (Hsubseteq) "[%Hfull Hregs]".
    pose proof (Hfull r) as Hlookup_r.
    iDestruct (big_sepM_union_acc  reg {[r := w]} (λ k v, k @@ i ->r v)%I with "Hregs") as "[Hsingle Hrestore]".
    {
  ...
  Lemma reg_bigM_split2 reg i r1 w1 r2 w2:
    r1 ≠ r2 ->
    reg !! r1 = Some w1 ->
    reg !! r2 = Some w2 ->
    ((full_reg_map reg ∗ [∗ map] k↦y ∈ reg, k @@ i ->r y)%I
     ⊢  (r1 @@ i ->r w1) ∗ (r2 @@ i ->r w2) ∗
          (∀ w1' w2', r1 @@ i ->r w1' ∗ r2 @@ i ->r w2'-∗ ∃ reg', (full_reg_map reg' ∗ [∗ map] k↦y ∈ reg', k @@ i ->r y)))%I.
  Proof.
