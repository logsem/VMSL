...
Section ftlr_msg_send.
  ...
  Lemma ftlr_msg_send {i mem_acc_tx ai regs ps_acc p_tx p_rx instr trans rx_state r0}:
  ...
  Proof.
    iIntros (Htotal_regs Hneq_0 Hsubset_mb Hsubset_acc Hnin_rx Hnin_tx Hlookup_PC Hin_ps_acc Hneq_ptx Hdom_mem_acc_tx Hin_ps_acc_tx
    ...
    { (* apply [msg_send_full_rx] *)
      ...
      exact Heq_decode. done. lia. 2: { iFrame "rx_state_v". } done.
      iNext. iIntros "(PC & mem_instr & pgt_acc  & tx & R0 & R1 & R2 & rx_state_v ) _".
      iDestruct ("Hacc_regs" $! (ai ^+ 1)%f with "[$ PC $ R0 $ R1 $ R2]") as (regs') "[%Htotal_regs' regs]".

      iDestruct ("Hacc_mem_acc_tx" with "[$mem_instr]") as "mem_acc_tx".
      iApply ("IH" $! _ ps_acc trans _ Htotal_regs' Hsubset_mb Hsubset_acc Hnin_rx Hnin_tx with "regs tx pgt_tx pgt_acc pgt_owned trans_hpool_global
                            tran_pgt_transferred retri R0z R1z R2z rx_state rx [other_rx rx_v rx_state_v rx_state_v'] prop0 propi tran_pgt_owned retri_owned [mem_rest mem_acc_tx mem_tx]
                            ").
      {
    ...
    { (* instruction is in ps_oea *)
      ...
      iApply (msg_send_secondary ai v (transaction_hpool_global_transferred trans ∗
               ...
               with "[PC R0 R1 R2 R0z R1z R2z pgt_acc tx mem_tx mem_instr rx_v rx_state_v mem_rx_v other_rx prop0 propi
                    trans_hpool_global tran_pgt_transferred mem_tran retri rx_state rx]"); iFrameAutoSolve.
      { solve_finz + n0. }
      {
        ...
        2: {
          iFrame.
          rewrite /rx_pages difference_difference_L. iFrame.
        }
        ...
        iSplitL "trans_hpool_global trans_pgt_transferred retri R0z R1z R2z rx_state rx other_rx mem_tran mem_rx_v rx_v rx_state_v propi".
        iExists trans. iFrame "trans_hpool_global trans_pgt_transferred retri rx".
        ...
        simpl. rewrite difference_difference_L.
        iFrame "other_rx".
        ...
        iExists (list_to_map (zip (finz.seq p_rx_v (length des)) des) ∪ mem_rx_v). iFrame.
        iExact "propi".
    ...
    { (* tpa ai ∉ ps_acc ∖ ({[p_rx]} ∪ ps_mem_in_trans) *)
      ...
      iApply (msg_send_secondary ai v (transaction_hpool_global_transferred trans ∗ transaction_pagetable_entries_transferred i trans
                                       ∗ retrievable_transaction_transferred i trans ∗ rx_pages (all_vmids ∖ {[i;v]})) _
               with "[PC R0 R1 R2 R0z R1z R2z pgt_acc tx mem_tx mem_instr rx_v rx_state_v mem_rx_v other_rx prop0 propi
                    trans_hpool_global tran_pgt_transferred Hacc_mem_inters mem_rest retri rx_state rx]"); iFrameAutoSolve.
      { solve_finz + n0. }
      {
        iSplitL "prop0". iFrame.
        iSplitL "propi". iFrame.
        ...
        2: {
          iFrame.
          rewrite /rx_pages. rewrite difference_difference_L. iFrame.
        }
        iNext. iIntros "((PC & mem_instr & pgt_acc & R0 & R1 & R2 & R0z & R1z & R2z & tx & #rx_v
        ...
        { set_solver +. }
        rewrite Heq_inter_diff_union.
        iSplitL "trans_hpool_global trans_pgt_transferred retri R0z R1z R2z rx_state rx other_rx mem_tran rx_v rx_state_v mem_rx_v propi".
        iExists trans. iFrame "trans_hpool_global trans_pgt_transferred retri rx".
        ...
        iApply (big_sepS_delete _ _ v with "[rx_v rx_state_v other_rx]");auto.
        simpl. rewrite difference_difference_L. iFrame "other_rx".
        iExists p_rx_v. iFrame "rx_v".
        iExists (Some (r2, i)). iFrame.
        iIntros "%". done.
        iSplitR "propi". iRight. iFrame.
        iSplitL "rx_state". iExists rx_state. iFrame.
        iExists v, p_rx_v, r2. iFrame.
        iSplitL "";auto. iFrame "rx_v".
        iSplitL "R1z". iExists r1. iFrame. done.
