...
Lemma p_upd_own_current_vm σ i (ps: gset PID):
 get_current_vm (update_ownership_global σ i ps) = get_current_vm σ.
Proof. f_equal. Qed.

Lemma p_upd_own_regs σ i (ps: gset PID):
 get_reg_files (update_ownership_global σ i ps) = get_reg_files σ.
Proof. f_equal. Qed.

Lemma p_upd_own_mem σ i (ps: gset PID):
 get_mem (update_ownership_global σ i ps) = get_mem σ.
Proof. f_equal. Qed.

Lemma p_upd_own_mb σ i (ps: gset PID):
 get_mail_boxes (update_ownership_global σ i ps) = get_mail_boxes σ.
Proof. f_equal. Qed.

Lemma p_upd_own_trans σ i (ps: gset PID):
 get_transactions (update_ownership_global σ i ps) = get_transactions σ.
Proof. f_equal. Qed.

Lemma p_upd_own_access σ i ps:
((get_access_gmap (update_ownership_global σ i ps)):gmap _ _) = ((get_access_gmap σ): gmap _ _).
Proof.
  rewrite /get_access_gmap /update_ownership_global  /=.
...
Lemma p_grnt_acc_current_vm σ i (ps: gset PID) :
 get_current_vm (grant_access_global σ i ps) = get_current_vm σ.
Proof. f_equal. Qed.

Lemma p_grnt_acc_regs σ i (ps: gset PID):
 get_reg_files (grant_access_global σ i ps) = get_reg_files σ.
Proof. f_equal. Qed.

Lemma p_grnt_acc_mem σ i (ps: gset PID):
 get_mem (grant_access_global σ i ps) = get_mem σ.
Proof. f_equal. Qed.

Lemma p_grnt_acc_mb σ i (ps: gset PID):
 get_mail_boxes (grant_access_global σ i ps) = get_mail_boxes σ.
Proof. f_equal. Qed.

Lemma p_grnt_acc_trans σ i (ps: gset PID):
 get_transactions (grant_access_global σ i ps) = get_transactions σ.
Proof. f_equal. Qed.

Lemma p_grnt_acc_ownerships σ i ps:
 (get_owned_gmap (grant_access_global σ i ps)) = (get_owned_gmap σ).
Proof.
  rewrite /get_owned_gmap /grant_access_global  /=.
  generalize dependent σ.1.1.1.2.
  induction ps as [|p ps] using set_ind_L.
  done.
  intro.
...
Lemma p_rvk_acc_current_vm σ i l:
  get_current_vm (revoke_access_global σ i l) = get_current_vm σ.
Proof. f_equal. Qed.

Lemma p_rvk_acc_regs σ i l:
  get_reg_files (revoke_access_global σ i l) = get_reg_files σ.
Proof. f_equal. Qed.

Lemma p_rvk_acc_mem σ i l:
  get_mem (revoke_access_global σ i l) = get_mem σ.
Proof. f_equal. Qed.

Lemma p_rvk_acc_mb σ i l:
  get_mail_boxes (revoke_access_global σ i l) = get_mail_boxes σ.
Proof. f_equal. Qed.

Lemma p_rvk_acc_trans σ i l:
  get_transactions (revoke_access_global σ i l) = get_transactions σ.
Proof. f_equal. Qed.

Lemma p_rvk_acc_ownerships σ i ps:
 (get_owned_gmap (revoke_access_global σ i ps)) = (get_owned_gmap σ).
Proof.
  rewrite /get_owned_gmap /revoke_access_global  /=.
  generalize dependent σ.1.1.1.2.
  induction ps as [|p ps] using set_ind_L.
  done.
  intro.
