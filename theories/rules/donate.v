...
Proof.
  ...
  iModIntro.
  iDestruct "Hσ" as "(Hcur & Hσmem & Hσreg & Hσtx & ? & ? & Hσowned & Hσaccess & Hσexcl & Htrans & Hσhp & %Hdisj & Hrcv)".
  (* valid regs *)
...
Lemma hvc_donate_z {instr i wi r2 pi ptx sown q sacc sexcl des sh wss} {l :Word} {spsd: gset PID}
  ...
  ∗ ▷ O@i:={q}[sown] ∗ ▷ A@i:={1}[sacc] ∗ ▷ E@i:={1}[sexcl]
  ∗ ▷ (R0 @@ i ->r r0) ∗ ▷ (R1 @@ i ->r r1) ∗ ▷(R2 @@i ->r r2) ∗  ▷ TX@ i := ptx
  ∗ ▷ mem_region des ptx ∗ ▷ ([∗ list] p;ws ∈ psd;wss, mem_page ws p)
  ∗ ▷ hp{ 1 }[ sh ] }}}
   ExecI @ i {{{ RET ExecI ; PC @@ i ->r (ai ^+ 1)%f ∗ ai ->a wi
  ∗ O@i:={q}[sown] ∗ A@i:={1}[sacc∖spsd] ∗ E@i:={1}[sexcl∖spsd]
  ∗ R0 @@ i ->r (encode_hvc_ret_code Succ) ∗ R1 @@ i ->r r1  ∗ TX@ i := ptx
  ...
  - (* step *)
    ...
    (* update regs *)
     ...
     rewrite !update_reg_global_update_reg ?update_access_batch_preserve_regs
             ?zero_pages_preserve_reg ?insert_transaction_preserve_regs;try solve_reg_lookup.
     2 : {
        exists r2.
        rewrite lookup_insert_ne.
        solve_reg_lookup.
     ...
     2 : {
        ...
        rewrite ?update_access_batch_preserve_regs
                ?zero_pages_preserve_reg
                ?insert_transaction_preserve_regs.
        exists r2.
        rewrite lookup_insert_ne;[solve_reg_lookup|done].
        rewrite ?update_access_batch_preserve_regs
                ?zero_pages_preserve_reg
     ...
     iDestruct ((gen_trans_update_insert h i W1 j psd Donation) with "Htrans") as ">[Hσtrans Htran]".
     { apply not_elem_of_dom.
       rewrite get_trans_gmap_preserve_dom.
       set_solver.
     }
