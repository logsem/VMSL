...
Lemma  mem_send_nz_not_share_update{n i ai r0 r1 r2 ptx sh h fhs j spsd } {l:Word} σ1 (tt: transaction_type):
...
Proof.
  ...
  (* valid pt *)
  iDestruct (access_agree_excl_check_true_bigS spsd with "Hσaccess Hacc") as %Hacc.
  ...
  iDestruct ((hpool_update_minus h ) with "Hhp Hσhp") as ">[Hσhp Hhp]".
  ...
  2: rewrite p_upd_pc_pgt !p_upd_reg_pgt //.
  assert (inv_trans_pgt_consistent (revoke_access_global (alloc_transaction σ1 h (i, W0, j, spsd, tt, false)) i spsd)).
  {
    ...
    { subst i0.
      ...
      rewrite /check_excl_access_page /= in Hacc.
      destruct (σ1.1.1.1.2 !! p) eqn:Hlookup.
      rewrite Hlookup in Hacc.
      destruct p0.
      rewrite Hlookup.
...
Proof.
  ...
  (* valid hpool *)
  iDestruct (gen_hpool_valid with "Hhp Hσhpool") as %Hhp.
  iSplit.
  - (* reducible *)
    iPureIntro.
  ...
  - (* step *)
    ...
    rewrite Hcur /=  Htx (@transaction_descriptor_valid _ _ i j W0 l psd σ1 des) /= in Heqc2;eauto.
    ...
    assert (forallb (λ v' : PID, check_excl_access_page σ1 i v' && check_ownership_page σ1 i v') psd = true) as HCheck.
    {
      apply forallb_forall.
      intros p HpIn.
      apply elem_of_list_In in HpIn.
      apply (elem_of_list_to_set (C:= gset PID)) in HpIn.
      rewrite <- Hspsd in HpIn.
      apply andb_true_iff.
      split.
      by apply Hacc.
      by apply Hown.
    ...
    { (* TODO *)
      iDestruct (mem_send_nz_not_share_update with "PC Hai Hown Hacc R0 R1 R2 Hhp
