...
Lemma str {i w1 w2 w3 q prx} ai a ra rb s:
  ...
  {SS{{ ▷ (PC @@ i ->r ai) ∗
    ...
    {{{ RET ExecI;
        ...
        RX@i := prx }}}.
Proof.
  iIntros (Hdecode Hnotrx Hs ϕ) "(>Hpc & >Hapc & >Hrb & >Harb & >Hra & >Hacc & >HRX) Hϕ".
  ...
  - (* step *)
    ...
    iApply "Hϕ".
    iFrame "Hpc Hapc Hrb Harb Hra Hacc HRX".
    apply (get_reg_gmap_get_reg_Some _ _ _ i) in HPC;eauto.
Qed.

Lemma str_error {i w1 w2 w3 s p} ai a ra rb :
  decode_instruction w1 = Some (Str ra rb) ->
  (to_pid_aligned a) ≠ p ->
  ...
  {SS{{ ▷ (RX@ i := p) ∗ ▷ (PC @@ i ->r ai) ∗ ▷ (ai ->a w1) ∗ ▷ (rb @@ i ->r a) ∗ ▷ (a ->a w3) ∗ ▷ (A@i:={1}[s]) ∗ ▷ (ra @@ i ->r w2)}}} ExecI @ i
                             ...
                             ∗ A@i:={1}[s] ∗ ra @@ i ->r w2 }}}.
Proof.
  iIntros (Hdecode Hs Has Hais ϕ) "(>Hrx & >Hpc & >Hapc & >Hrb & >Harb & >Hacc & >Hra ) Hϕ".
  iApply (sswp_lift_atomic_step ExecI);[done|].
  iIntros (σ1) "%Hsche Hσ".
  inversion Hsche as [ Hcur ]; clear Hsche.
  apply fin_to_nat_inj in Hcur.
