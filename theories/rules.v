...
Lemma yield {z i w1 w2 a_ b_ q} ai :
  ...
  ∗ R 0 R0Av @@ i ->r w2
  ∗ R 0 R0Av @@ z ->r (encode_hvc_ret_code Succ)
...
Proof.
  ...
  iDestruct (gen_reg_valid_global1 σ1 (R 0 R0Av) z a_ with "Hregown Hr1'") as "%Hr1'".
  iDestruct (gen_reg_valid_global1 σ1 (R 1 R1Av) z b_ with "Hregown Hr2'") as "%Hr2'".
  (* valid pt *)
  ...
  - iModIntro.
    ...
    + destruct Hstep as [Hstep1 Hstep2].
      ...
      iDestruct (gen_reg_update_Sep σ1
                                    ...
                                    {[(R 0 R0Av, z):= encode_hvc_ret_code Succ;
                                      (R 1 R1Av, z):= (encode_vmid (get_current_vm σ1));
      ...
      * rewrite !big_sepM_insert ?big_sepM_empty; eauto.
        ...
        -- iMod "Htok'".
           ...
           rewrite (map_union_comm {[(R 1 R1Av, z) := encode_vmid σ1.1.1.2]} {[(PC, σ1.1.1.2) := ai +w 1]}).
           rewrite map_union_assoc.
           rewrite (map_union_comm {[(R 0 R0Av, z) := encode_hvc_ret_code Succ]} {[(PC, σ1.1.1.2) := ai +w 1]}).
           rewrite <-(map_union_assoc {[(PC, σ1.1.1.2) := ai +w 1]}
                                      {[(R 0 R0Av, z) := encode_hvc_ret_code Succ]}
                                      {[(R 1 R1Av, z) := encode_vmid σ1.1.1.2]}).
           rewrite (map_union_comm {[(R 0 R0Av, z) := encode_hvc_ret_code Succ]}
                                   {[(R 1 R1Av, z) := encode_vmid σ1.1.1.2]}).
           rewrite !map_union_assoc.
           iAssumption.
