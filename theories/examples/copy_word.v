...
Section copy_word.
  ...
  Lemma c_loop {progpage sacc i q} {base : Imm} {n : nat} (body : list Word) step prog (P : Word -> iProp Σ) :
    ...
    (∀ v (v' : Word) (step : Imm), ⌜v = Z.to_nat (finz.to_z v') ∧ (v <= S n)⌝ -∗ {PAR{{ (P v') ∗ PC @@ i ->r ((of_pid progpage) ^+ 3)%f
                         ∗ R6 @@ i ->r v'
                         ∗ R5 @@ i ->r step
  ...
  Proof.
    ...
    - rename Hn into eq.
      ...
      iDestruct ("Htriple" $! 0 I0 step _ with "") as "#HprogSpec".
      Unshelve.
      2 : {
        split.
        rewrite /I0.
    ...
    - rename Hn into eq.
      ...
      rewrite Htemp' in Htemp.
      clear Htemp'.
      iSpecialize ("IH" $! im _ _ _).
      iIntros (Φ).
      iModIntro.
      iIntros "(HP & Hpc & [% Hr5] & [% Hr6] & [% Hr7] & [% Hr8] & [% Hnz] & Hacc & Hprog) HΦ".
      iApply parwp_parwp.
