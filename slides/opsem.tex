...
\begin{frame}
  ...
  \begin{mathpar}
    ...
    \inferrule[Exec-halt]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{halt})}
...
\begin{mathpar}
 ...
 \and \inferrule[Exec-hvc-Succ-Yield-Secondary]
...
\begin{frame}
  ...
  \begin{figure}
\begin{mathpar}
   \inferrule[Exec-hvc-Succ-MemMng-Succ]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    ...
    w_l = rs_v[\reg{R1}]\\
    \pid_t = \delta_v.mb.tx\\
    \text{ValidTransactionDescriptor}(pid_t,w_l,v_s,0,w_f,w_c,pgs,mm,\delta s,f)\\
    w_h \notin \text{dom}(sss)\\
    w_h[63]=1}
    ...
    \end{array}}\right\},\\mm, sss~\text{with}~\{[w_h]=(v,w_f,[],pgs,f)\})}
\end{mathpar}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}
\begin{mathpar}
  \inferrule[Exec-hvc-Succ-MemRetrieve-MemDonate]
    {...\\
    \text{ValidRetrieveDescriptor}(pid_t,w_l,v_s,w_f',w_h,v,mm)\\
    (v_s,w_f,vrs, pgs,\tt{MEM\_DONATE})=sss[w_h]\\
    (v,ps) \in pgs \\
    v\notin vrs\\
    \delta_v.mb.rx=(pid_r,\FALSE,-,-) }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow
      (\DNNXT{v} ,\,\\
      (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{MEM\_RETRIEVE\_RESP};\\
    \null[v].rs[\reg{R2}]=7+\text{len}(ps);\\
    \null \forall pid. pid\in ps \rightarrow [v].pt[pid]=(O,EA);\\
    \null \forall pid. pid\in ps \rightarrow [v_s].pt[pid]=(!O,NA);\\
    \null[v].mb.rx=(pid_r,\TRUE,7+\text{len}(ps),-)
    \end{array}}\right\},
    mm~\text{with}~\\\left\{{\begin{array}{l}
    pid_r[0]=\tw{v_s}; ~~\null pid_r[1]=(w_f|w_f') \& 0\text{x}18;\\
    \null pid_r[2]=0;~~
    \null pid_r[3]=1;~~ \null pid_r[4]=\tw{v};\\ \null pid_r[5]=6;pid_r[6]=\text{len}(ps);\\
    \null \forall i. i < \text{len}(ps) \rightarrow pid_r[7+i]=ps[i][0]
    \end{array}}\right\},\text{rm}(sss,w_h)))}
\end{mathpar}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Extend FFA with permissions
    \item Develop a program logic - to reason about client programs\\
      (Ownership in Iris is different from ownership in FFA...More like accessibility!)
    \item Investigate in security properties - memory isolation etc.
  \end{itemize}
\end{frame}
