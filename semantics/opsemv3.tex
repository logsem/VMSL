...
\begin{figure}[hbt!]
  ...
  \inferrule[Exec-hvc-Succ-MemRetrieveReq-Error-InvalidParams]
    ...
    \lor w_h \notin \text{dom}(sss) \\
    ((v_s',w_f',w_t',entry,fid)=sss[w_h] \rightarrow
    (\lor v_s' \ne \tv{mm[\addr(pid_t,0)]} \\
    \lor w_t'\ne mm[\addr(pid_t,3)] \\
    \lor w_f'[63:1]\ne 0 \\
    \lor w_c \ne 0 \\
...
\begin{figure}[hbt!]
  ...
  (v \ne \tv{mm[\addr(\pid_t,0)]} \lor  !\text{OwnedExclPage}(\delta_v, \pid_d))}
  {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
  [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
  \null[v].rs[\reg{R0}]=\efid{ERROR};\\
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    \pid_t=\delta_v.mb.tx\\ 
    !\text{ValidRetrieveDescriptor}(pid_t,w_l,v_s,w_f,w_h,w_t,mm)\lor w_h \notin \text{dom}(sss)}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    ...
    \text{IsFFA}(\delta_v,\tt{MEM\_RELINQUISH})\\
    \pid_t=\delta_v.mb.tx\\ 
    \text{ValidRetrieveDescriptor}(pid_t,w_l,v_s,w_f,w_h,w_t,mm)\\
    w_h \in \text{dom}(sss)\\
    ...
    \and \inferrule[Exec-hvc-MemRelinquish-Succ]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RELINQUISH})\\
    pid_t=\delta_v.mb.tx\\ 
