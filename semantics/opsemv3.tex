...
\begin{figure}[hbt!]
  ...
  (v \ne v_s \lor  !\text{OwnedExclPage}(\delta_v, \pid_d))}
  {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
  [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
  \null[v].rs[\reg{R0}]=\efid{ERROR};\\
  ...
  \end{array}}\right\},mm,sss))}
   ...
   \and \inferrule[Exec-hvc-Succ-MemDonate-Succ]
    ...
    \null[n].pt[pid_d]=(!O,NA)
    \end{array}}\right\},mm,\\ 
    sss~\text{with}~\{[w_h]=(v,w_f,w_t,inl\ (n,pid_d),\tt{MEM\_DONATE})\})}
...
\begin{figure}
  \begin{mathpar}    
   ...
   \and \inferrule[Exec-hvc-Succ-MemDonate-MemRetrieveResp-NotZeroed]
    ...
    w_h=mm[\addr(pid_t,2)]\\ 
    w_h[63]=1 \land w_h \in \text{dom}(sss) \\
    (v_s,w_f',mm[\addr(pid_t,3)], inl\ (v ,pid_d),\tt{MEM\_DONATE})=sss[w_h]\\
    v_s = \tv{mm[\addr(pid_t,0)]} \\ 
    mm[\addr(pid_t,1)]=0 \\ 
    mm[\addr(pid_t,4)]=0\\ 
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Succ-MemLend-Succ-NotZeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\ 
    w_l = rs_v[\reg{R1}]\\
    pid_t=\delta_v.mb.tx\\
    \text{ValidLendDescriptor}(pid_t,w_l,v,0,0,w_t,w_c,pgs,mm,\delta s)\\
    w_h \notin \text{dom}(sss) \\
    w_h[63]=1\\
    }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    ...
    \end{array}}\right\},mm,\\ 
    sss~\text{with}~\{[w_h]=(v,0,w_t,[],pgs,\tt{MEM\_LEND})\}))}

    \and \inferrule[Exec-hvc-Succ-MemLend-Succ-Zeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\
    w_l = rs_v[\reg{R1}]\\
    pid_t=\delta_v.mb.tx\\
    \text{ValidLendDescriptor}(pid_t,w_l,v,1,0,w_t,w_c,pgs,mm,\delta s)\\
    w_h \notin \text{dom}(sss) \\
    w_h[63]=1
    }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    ...
    sss~\text{with}~\{[w_h]=(v,1,w_t,[],pgs,\tt{MEM\_LEND})\}))}
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{SUCC};\\
    \null \forall pid. \exist v_r. pid\in pgs[v_r] \rightarrow [v].pt[pid]=(O,EA);\\
    \end{array}}\right\},\\
    mm, remove(sss, w_h))}
    ...
    \null \forall pid. \exist v_r. pid\in pgs[v_r] \rightarrow [v].pt[pid]=(O,EA);\\
    \end{array}}\right\},\\
    zero(mm,[pid|\exists v_r. pid\in pgs[v_r]]), \text{remove}(sss, w_h))}
...
\inferrule[Exec-hvc-Succ-MemShare-Succ-NotZeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_SHARE})\\
    w_l = rs_v[\reg{R1}]\\
    ...
    \null \forall pid. \exists v_r. pid \in pgs[v_r] \rightarrow [v_r].pt[pid]=(!O,NA)
    \end{array}}\right\},mm,\\
    sss~\text{with}~\{[w_h]=(v,0,w_t,[],pgs,\tt{MEM\_SHARE})\}))}

    \and \inferrule[Exec-hvc-Succ-MemShare-Succ-Zeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\
    w_l = rs_v[\reg{R1}]\\
    ...
    \null \forall pid. \exists v_r. pid \in pgs[v_r] \rightarrow [v_r].pt[pid]=(!O,NA)\end{array}}\right\},
    \text{zero}(mm,[pid| \exists v_r. pid\in pgs[v_r]]),\\
    sss~\text{with}~\{[w_h]=(v,1,w_t,[],pgs,\tt{MEM\_SHARE})\}))}
