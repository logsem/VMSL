...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    w_h=mm[\addr(\pid_t,0)]\\ 
    w_f=mm[\addr(\pid_t,1)]\\
    (((v_s,w_f',w_t', inl\ (v_r, \pid_d), fid)=sss[w_h] \rightarrow (v_r \ne v \lor [v_r].pt[\pid_d] \ne (!O, EA))) \\\lor
    ((v_s,w_f',w_t', inr\ (S n, ls), fid)=sss[w_h] \land \exists i. i \in ls, i = (v_r, \pid_d) \rightarrow (v_r \ne v \lor [v_r].pt[\pid_d] \ne (!O, SA))))}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    ...
    \and \inferrule[Exec-hvc-MemRelinquish-Succ-One-Borrower]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RELINQUISH})\\
    pid_t=\delta_v.mb.tx\\ 
    ...
    w_f=mm[\addr(pid_t,1)]\\
    w_h \in \text{dom}(sss) \\
    (v_s,w_f',w_t', inl\ (v_r, pid_d), fid)=sss[w_h]\\
    [v_r].pt[pid_d] = (!O, EA)}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
