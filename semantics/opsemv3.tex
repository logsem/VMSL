...
\begin{figure}[hbt!]
  ...
  \inferrule[Exec-hvc-Succ-MemRetrieveReq-Error-InvalidParams]
    ...
    \lor w_h=0 \\
    \lor w_h \notin \text{dom}(sss) \\
    \lor ((v_s',w_f',w_t',entry,fid)=sss[w_h] \rightarrow
    (v_s' \ne \tv{v_s} \\
    \lor w_t'\ne w_t \\
    \lor w_f'[63:1]\ne 0 \\
    \lor w_c \ne 0 \\
    \lor (inr\ ls=entry \land forall (v_r, pid_d). (v_r, pid_d) \in ls \rightarrow v_r\ne v) \\
    \lor (inl\ (v_r, pid_d) = entry \land v_r \ne v)))}
    ...
    \and \inferrule[Exec-hvc-Succ-MemRetrieveReq-Error-Busy]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    ...
    w_f'[63:1]=0 \\
    (v_s',w_f',w_t',entry,fid)=sss[w_h]\\
    mm[\addr(pid_t,4)]=0\\ 
    inr\ (n, ls) =entry \rightarrow exists (v_r, pid_d). (v_r, pid_d) \in ls \land v_r= v\\ 
    inl\ (v_r, pid_d) = entry \rightarrow v_r = v \\ 
    \delta_v.mb.rx=(-,\TRUE,-,-) }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
...
\begin{figure}[hbt!]
  ...
  w_l = rs_v[\reg{R1}]\\
  \pid_t = \delta_v.mb.tx\\
  !ValidDonateDescriptor(pid_t,w_l,v_s,w_f,0,_,w_c,pgs,mm,\delta s)
  }
  {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
  [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
  \null[v].rs[\reg{R0}]=\efid{ERROR};\\
  ...
  \and \inferrule[Exec-hvc-Succ-MemDonate-Error-Denied]
  {\text{AccessibleAddr}(\delta_v,a_i) \\
  \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
  \text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\
  w_l = rs_v[\reg{R1}]\\
  ...
  \end{array}}\right\},mm,sss))}
   ...
   \and \inferrule[Exec-hvc-Succ-MemDonate-Succ]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ 
    w_l = rs_v[\reg{R1}]\\
    \pid_t = \delta_v.mb.tx\\
    ValidDonateDescriptor(pid_t,w_l,v_s,w_f,0,_,w_c,pgs,mm,\delta s)
    (v = v_s \lor \forall pid. \exists v_r. pid\in pgs[v_r] \land \text{OwnedExclPage}(\delta_v, pid))
    w_h \notin \text{dom}(sss)}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{SUCC};\\
    ...
    \end{array}}\right\},mm,\\ 
    sss~\text{with}~\{[w_h]=(v,w_f,w_t,[],pgs,\tt{MEM\_DONATE})\})}
    
    \end{mathpar}
\end{figure}

\begin{align*}
  \text{ValidRetrieveDescriptor}(p,w_l,v_s,w_f,w_h,w_t,mm)\defined &~~\\
  v_{s}=\tv{mm[p[0]]} \land w_{f}=mm[p[1]] \land w_{h}=mm[p[2]] \land &~~//\text{main fields}\\
   w_{t} = mm[p[3]] \land mm[p[4]]=0 \land &~~ \\
  w_l \le \PPMAX \land w_{l} = 5  &~~//\text{validate length}
\end{align*}

\begin{figure}
  \begin{mathpar} 
    
    \inferrule[Exec-hvc-Succ-MemDonate-MemRetrieveResp-NotZeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    w_l=rs_v[\reg{R1}]\\
    pid_t=\delta_v.mb.tx\\ 
    \text{ValidRetrieveDescriptor}(pid_t,w_l,v_s,0,w_h,w_t,mm)\\
    w_h \in \text{dom}(sss) \\
    ...
    \null[v].mb.rx=(pid_r,\TRUE,7+\text{len}(ps),-)
    \end{array}}\right\},\\
    ...
    \null pid_r[5]=\tw{v};\\
    \null pid_r[6]=7;\\
    \null \forall i. i < \text{len}(ps) \rightarrow pid_r[7+i]=ps[i][0]
    \end{array}}\right\},sss with\{[w_h]=(v_s,w_f,w_t,v::vrs, pgs,\tt{MEM\_DONATE})\})}
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Succ-MemLend-Error-InvalidParams]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\ 
    w_l = rs_v[\reg{R1}]\\
    \pid_t = \delta_v.mb.tx\\
    !\text{ValidDescriptor}(pid_t,w_l,v_s,w_f,0,_,w_c,pgs,mm,\delta s) \lor w_f[63:1] \ne 0 }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    \null[v].rs[\reg{R2}]=\eec{INV\_PARA}
    ...
    \and \inferrule[Exec-hvc-Succ-MemLend-Error-Denied]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\ 
    w_l = rs_v[\reg{R1}]\\
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Succ-MemLend-Succ]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\ 
    w_l = rs_v[\reg{R1}]\\
    w_l\le\PPMAX\\
    pid_t=\delta_v.mb.tx\\ 
    \text{ValidLendDescriptor}(pid_t,w_l,v,w_f,0,w_t,w_c,pgs,mm,\delta s)\\
    w_h \notin \text{dom}(sss) \\
    w_h[63]=1}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    ...
    \null \forall pid. \exists v_r. pid \in pgs[v_r] \rightarrow [v_r].pt[pid]=(!O,NA)
    \end{array}}\right\},mm,\\ 
    sss~\text{with}~\{[w_h]=(v,w_f,w_t,[],pgs,\tt{MEM\_LEND})\}))}
 
    % \and \inferrule[Exec-hvc-Succ-MemLend-Succ-Multiple-Borrowers]
    % {\text{AccessibleAddr}(\delta_v,a_i) \\
    % \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    % \text{IsFFA}(\delta_v,\tt{MEM\_LEND})\\
    % w_l = rs_v[\reg{R1}]\\
...
\begin{figure}
  \begin{mathpar} 
    ...
    \inferrule[Exec-hvc-Succ-MemLend-MemRetrieveResp-NotZeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    w_l=rs_v[\reg{R1}]\\
    ...
    \and \inferrule[Exec-hvc-Succ-MemLend-MemRetrieveResp-Zeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    w_l=rs_v[\reg{R1}]\\
...
%     w_h=mm[\addr(pid_t,2)]\\
%     w_h[63]=1 \land w_h \in \text{dom}(sss) \\
%     (v_s,w_f',mm[\addr(pid_t,3)], inr\ (n, ls),\tt{MEM\_LEND})=sss[w_h]\\
...
%     \end{array}}\right\},\\
%     sss[w_h] = (v_s,w_f',mm[\addr(pid_t,3)], inr\ (n+1, ls),\tt{MEM\_LEND})))}

%     \inferrule[Exec-hvc-Succ-MemLend-MemRetrieveResp-Zeroed-Multiple-Borrowers]
%     {\text{AccessibleAddr}(\delta_v,a_i) \\
%     \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
%     \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
%     rs_v[\reg{R1}]\le\PPMAX\\
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    v \notin vrs \lor (\exists pid. pid\in pgs[v] \rightarrow [v].pt[pid] \notin \{(!O, EA),(!O,SA)\})}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    ...
    (v_s,w_f,w_t,vrs, pgs,f)=sss[w_h] \\
    v \in vrs \\
    \forall pid. pid\in pgs[v] \rightarrow [v].pt[pid] \in \{(!O, EA),(!O,SA)\}}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{SUCC};\\
    ...
    % \and \inferrule[Exec-hvc-MemRelinquish-Succ-Multiple-Borrowers]
    % {\text{AccessibleAddr}(\delta_v,a_i) \\
    % \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    % \text{IsFFA}(\delta_v,\tt{MEM\_RELINQUISH})\\
    % pid_t=\delta_v.mb.tx\\
    ...
    % w_f=mm[\addr(pid_t,1)]\\
    % w_h \in \text{dom}(sss) \\
    % (v_s,w_f',w_t', inr\ (S n, ls), fid)=sss[w_h]\\
    ...
    % \null[v].pt[pid_d]=(!O,NA);\\
    % \end{array}}\right\},\\
    % mm, sss[w_h]=(v_s,w_f',w_t', inr\ (n, ls), fid))}
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Reclaim-Error-Denied-TODO]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RECLAIM})\\
    w_h=comb([v].rs[\reg{R1}], [v].rs[\reg{R2}])\\ 
    (((v,w_f',w_t', inl\ (v_r, \pid_d), fid)=sss[w_h] \rightarrow [v].pt[\pid_d] \ne (O, NA)) \lor \\
    (v,w_f',w_t', inr\ (0, ls), fid)=sss[w_h] \rightarrow [v].pt[\pid_d] \ne (O, SA)))}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    ...
    w_f = 0\\
    w_h \in \text{dom}(sss) \\
    (((v,w_f',w_t', inl\ (v_r, pid_d), fid)=sss[w_h] \land [v].pt[pid_d] = (O, NA)) \lor \\
    (v,w_f',w_t', inr\ (0, ls), fid)=sss[w_h] \land [v].pt[pid_d] = (O, SA)))}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow\\ 
    (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{SUCC};\\
    ...
    \null[v].pt[pid_d]=(O,EA);\\
    \end{array}}\right\},\\
    \text{zero}(mm,pid_d)~, remove(sss, w_h))}
...
\begin{figure}[hbt!]
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Succ-MemShare-Error-InvalidParams]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    ...
    w_c = mm[\addr(\pid_t, 4)]\\
    n = \tv{mm[\addr(\pid_t, 5)]}\\ 
    (w_l > len(\pid_t) \lor w_c \neq 1 \lor w_l \neq 6 \lor !\text{ValidVMID}(\delta s, n) \lor n = v \lor w_f[63:1] \ne 0 \lor w_h \ne 0 \\\lor w_l \neq 4 + 2 * w_c \lor w_f[63:1] \ne 0 \lor w_h \ne 0 \\\lor \exists n. n < w_c \land ! \text{ValidVMID}(\delta s, (\tv{mm[\addr(\pid_t, 4 + 2*n - 1)]})) \lor (\tv{mm[\addr(\pid_t, 4 + 2*n - 1)]}) = v)}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    \null[v].rs[\reg{R2}]=\eec{INV\_PARA}
    ...
    \and \inferrule[Exec-hvc-Succ-MemShare-Error-Denied]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_SHARE})\\ 
    w_l = rs_v[\reg{R1}]\\
    \pid_t=\delta_v.mb.tx\\ 
    w_c = mm[\addr(\pid_t, 4)]\\
    (v \ne \tv{mm[\addr(\pid_t, 0)]} \lor \exists n. n < w_c \land !\text{OwnedExclPage}(\delta_v, \pid(mm[\addr(\pid_t, 4+2*n)])))}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}
    [v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\
    \null[v].rs[\reg{R0}]=\efid{ERROR};\\
    \null[v].rs[\reg{R2}]=\eec{DENIED}
    ...
    \and \inferrule[Exec-hvc-Succ-MemShare-Succ]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_SHARE})\\ 
    w_l = rs_v[\reg{R1}]\\
    ...
    sss~\text{with}\\
    \null ~\{[w_h]=(v,w_f,mm[\addr(pid_t,3)],\\
    \null inr\ (0, [\tv{mm[\addr(pid_t,4 + 2*i - 1)]},\pid(mm[\addr(pid_t,4 + 2*i)])\ |\ i \leftarrow 0 \dots w_c]),\tt{MEM\_SHARE})\})}   
...
\begin{figure}
  \begin{mathpar}
    ...
    \inferrule[Exec-hvc-Succ-MemShare-MemRetrieveResp-NotZeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    rs_v[\reg{R1}]\le\PPMAX\\
    ...
    w_h=mm[\addr(pid_t,2)]\\ 
    w_h[63]=1 \land w_h \in \text{dom}(sss) \\
    (v_s,w_f',mm[\addr(pid_t,3)], inr\ (n, ls),\tt{MEM\_SHARE})=sss[w_h]\\
    v_s = \tv{mm[\addr(pid_t,0)]} \\ 
    mm[\addr(pid_t,1)]=0 \\ 
    mm[\addr(pid_t,4)]=0\\ 
    exists i. i \in ls. i = v \\ 
    ...
    \null[\addr(pid_r,6)]=\addr(pid_d,0)
    \end{array}}\right\},\\
    sss[w_h] = (v_s,w_f',mm[\addr(pid_t,3)], inr\ (n+1, ls),\tt{MEM\_SHARE})))}
  
    \and \inferrule[Exec-hvc-Succ-MemShare-MemRetrieveResp-Zeroed]
    {\text{AccessibleAddr}(\delta_v,a_i) \\
    \text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\
    \text{IsFFA}(\delta_v,\tt{MEM\_RETRIEVE\_REQ})\\
    rs_v[\reg{R1}]\le\PPMAX\\
