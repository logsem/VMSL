...
\begin{figure}
  \begin{align*}
    ...
    \delta &\in \STATE &\defined &list(\PID) \times \REGS \times \MB \\% \TIMER \times
...
messages expect the scheduler to run a specific VM, usually either the
sender or the receiver, next. The purpose is to get the sender or the
receiver notified.
...
working like the timer interrupt(not an actual interrupt!) in our model. The $\TIMER$ countdowns by $1$
whenever executing an instruction. When its value becomes zero, the currently running VM
will be preempted, switching back to the primary VM, and
meanwhile, notifying the primary VM who was running before the ``interrupt''.
These tasks are done by the tick function. One may
notice that the notification is carried out by assigning values to registers of
primary VM, which is exactly the foregoing hvc call conventions.
...
the hypervisor. The hypervisor will check if the caller is the primary VM,
and whether the given $\VMID$ is valid. Next, the hypervisor examines the state of
$\MB$ of the next running VM. If the message in $\MB$ is ready to read, the
hypervisor additionally sets parameter registers of that VM, notifying the VM
along with the sender and the length of the message.
\todo{the pvm is responsible
for setting the timer when scheduling, to do so the timer must be a register. Or it is ok to let
hypervisor do it?}
...
of this function is trivial, it just informs the scheduler the previous running VM just
...
The MEG\_WAIT is used by a VM to wait for a message. While in
...
control is given back to the VM, notifying it in the same manner as RUN.
