...
threads. The first approach follows from the fact that the number of VMs in the
system is fixed. Therefore, we just need a fixed size thread pool by removing
the capability to create new threads(e.g. the \texttt{fork} expression in
...
The consequence of removing folking ability from the operational semantics is
...
iterated separated conjunction(the ``big star'') that describing the safey of
forked off threads. One may think it is fine losing the big star, while it is
the key to ``compose'' proofs of threads together, which means even if we could
still achieve so called vm local reasoning, we can only reason about VMs
seperately! There would be no way to transfer/share resources/invariants between
VMs.
One possible solution that we have discussed was adding a
trivial weakestpre for the next vm to run. For instance, if $\beta$ is
$\mathsf{Yield}~j$, then we need to show
