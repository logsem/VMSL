...
\begin{figure}
  \begin{align*}
    ...
    t & \in \PT & \defined & \PID \maps (\OS \times \AS) \\
...
\begin{figure}[h!]
  \begin{mathpar}
    ...
    \and
    \begin{aligned}
      \valida(\Delta,v,r,P) &=\left\{ \begin{array}{ll}
                                      \SOME(a_i) & \text{if} \; a_i = \Delta[v].r[r] \\
                                                 &\land \big( (P=V \land \Delta[v].t[\pid(a_i)]\ne (-,NA)) \\
                                                 &\lor (P=R \land (\Delta[v].t[\pid(a_i)] \ne(-,NA) \lor \SOME(\pid(a_i),-,-,-)=\Delta[v].\pi_r)) \\
                                                 &\lor (P=W \land (\Delta[v].t[\pid(a_i)] \ne(-,NA)\lor \SOME(\pid(a_i))=\Delta[v].\pi_t))\big) \\
                                    \NONE & \text{otherwise}
                                  \end{array}
                                \right. \\
      \pid(a) &= a >> \PPBITS
