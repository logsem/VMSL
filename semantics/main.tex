...
\begin{figure}[h!]
  \begin{adjustwidth}{-1cm}{}
    \begin{tabular}[t]{L|L L| L}
      ...
      \instrm{fail}& \leftshift  (\DN{\mathtt{Fail}}, (\Delta,m,s)) & & \\ \hline
      \instrm{halt}&\leftshift   (\DN{\mathtt{Halt}}, (\Delta,m,s))  & & \\\hline
      \instrm{br} \; r  &\leftshift \nextt(\Delta',m,s,\text{id},v,v)  & \Delta'[v].rs[\mathtt{pc}]=\Delta[v].rs[r]  &  \\\hline
      \instrm{bne} \; r  & \leftshift \nextt(\Delta',m,s,\text{id},v,v) & \leftshift \begin{array}{l} \Delta'[v].rs[\mathtt{pc}]= \\ \;\;\;\;\left\{
      ...
      \instrm{mov}\; r\; w  &\leftshift \nextt(\Delta', m, s,\updpc,v,v)  & \Delta'[v].rs[r]=w & \\\hline
      \instrm{ldr}\; r_{1} \; r_{2}  &\leftshift \nextt(\Delta', m, s,\updpc,v,v) & \Delta'[v].rs[r_{1}]=m[a]  &
                                \SOME(a)=\valida(\Delta[v],r_{2}) \\\hline
      \instrm{str}\; r_{1} \; r_{2}  &\leftshift \nextt(\Delta', m, s,\updpc,v,v) & m'[a]=\Delta[v].rs[r_{2}]&
                                \SOME(a) =\valida(\Delta[v],r_{1}) \\\hline
      \instrm{add}\; r_{1}\; r_{2}\; r_{3} &\leftshift \nextt(\Delta', m, s,\updpc,v,v) &\leftshift \begin{array}{l}\Delta'[v].rs[r_{1}]=\\  \Delta[v].rs[r_{2}] + \Delta[v].rs[r_{3}] \end{array} & \\\hline
      ...
      \instrm{memd}\; r_{1}\; r_{2} &\leftshift \nextt(\Delta', m, s',\updpc,v,v)
                                 &\!\!\!\!\! \begin{array}{l}
                                               ...
                                               \Delta'[v].rs[\mathtt{r_{0}}]=\llen{s'}\\
                                               r =  \Delta[v].rs[r_{2}] \\
                                               s' = s \append [(\pid(a_p), \Delta[v].rs[r_{2}])]
                                             \end{array}
                      &\leftshift\begin{array}{l}
      ...
      \instrm{memr}\; r &\leftshift \nextt(\Delta', m, s',\updpc,v,v)
                      ...
                      &\leftshift\begin{array}{l}
                                   ...
                                   \land p \notin \Delta[v].ps
                                 \end{array} \\\hline
      \instrm{run}\; r & \leftshift \nextt(\Delta', m, s,\updpc,v, v_n)
                                 &\!\!\!\!\!\begin{array}{l}
                                              v_n=\Delta[v].rs[r_0]\\
                                              \SOME(p_r,b,w_s,v_s)=\Delta[v].\pi\\
                                              \!\!\!\!\!\left.\begin{array}{l}
                                              \Delta'[v_n].rs[\mathtt{r_0}]=\MSGS\\
                                              \Delta'[v_n].rs[\mathtt{r_1}]=v_s\\
                                              \Delta'[v_n].rs[\mathtt{r_3}]=w_s
                                                        \end{array}\!\!\!\!\!\right\} \text{if } b
                                              \end{array}
                                    & v=0 \land v_n < \llen{\Delta} \\\hline
      \instrm{yld} & \leftshift \nextt(\Delta', m, s,\updpc, v, 0)
                                 &\!\!\!\!\! \begin{array}{l}
                                               \Delta'[0].rs[\mathtt{r_0}]=\YIELD\\
                                               \Delta'[0].rs[\mathtt{r_1}]=v
                                               \end{array}& \\\hline
      \instrm{msgw} & \leftshift \nextt(\Delta', m, s,\updpc, v, v_n)
                                 &\!\!\!\!\!\begin{array}{l}
                                              \SOME(p_r,b,w_s,v_s)=\Delta[v].\pi\\
                                              \!\!\!\!\!\left.\begin{array}{l}
                                              v_n=v\\
                                              ...
                                              \!\!\!\!\!\left.\begin{array}{l}
                                              v_n=0\\
                                              \Delta'[v_n].rs[\mathtt{r_0}]=\MSGW\\
                                              \Delta'[v_n].rs[\mathtt{r_1}]=v
                                                              \end{array}\!\!\!\!\!\right\} \text{if } !b\\
                                              \end{array} & \\\hline
      \instrm{msgs}\; r_1 \; r_2 & \leftshift \nextt(\Delta', m', s,\updpc,v,v_n)
                                 &\!\!\!\!\!\begin{array}{l}
                                              v_r=\Delta[v].rs[r_1]\\
                                              w_s=\Delta[v].rs[r_2]\\
                                              \SOME(p_r,b,-,-)=\Delta[v_r].\pi\\
                                              \!\!\!\!\!\left.\begin{array}{l}
                                              v_n=0\\
                                              \Delta'[v_r].\pi=\SOME(p_r,\mathtt{true},w_s,v)\\
                                              m'=\text{memcpy}(m,p_t,p_r,w_s)\\
                                              \Delta'[v].rs[\mathtt{r_0}]=\SUCC\\
                                              \Delta'[0].rs[\mathtt{r_0}]=\MSGS\;\text{if}\;v\neq 0\\
                                              \Delta'[0].rs[\mathtt{r_1}]=\text{comb}(v,p_r)\;\text{if}\;v\neq 0
                                                              \end{array}\!\!\!\!\!\right\} \text{if}\;!b\\
                                              \!\!\!\!\!\left.\begin{array}{l}
                                                                v_n=v\\
                                                                \Delta'[v].rs[\mathtt{r_0}]=\BUSY
                                                              \end{array}\!\!\!\!\!\right\}\;\text{if}\;b
                                            \end{array}
                      &\SOME(p_t)=\Delta[v].tx \\\hline
      \instrm{msgr} & \leftshift  \nextt(\Delta', m, s,\updpc, v, v_n)& \Delta'[v].\pi = \SOME(p_r,\mathtt{false},-,-) &\Delta[v].\pi=\SOME(p_r,\mathtt{true},-,-) \\\hline
    \end{tabular}
  \end{adjustwidth}
  \caption{Instruction Semantics.}
\end{figure}
\todo{change notation: rs ->r, tx -> ?}
