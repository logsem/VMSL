...
$\PID$s as the abstract page table of a VM. % \todo{Lars: I don't understand
this para} We argue it is sufficient to record all the necessary translation
...
\begin{figure}[h!]
  \begin{mathpar}
    ...
    {{(\EI{v},\, \phi) \rightarrow \left\{
          \begin{array}{ll}
            \sem{\decode(i_w)}(\phi.\Delta, \phi.m, \phi.s ,v)
            & \mathrm{if} \;\SOME(i_a) = \valida(\phi.\Delta[v], \mathtt{pc}) \land m[i_a]=i_w \\
    ...
    \begin{aligned}
      \valida(\delta,r) &=\left\{ \begin{array}{ll}
                                    \SOME(i_a) & \text{if} \; i_a = \delta.rs[r] \land \pid(i_a) \in \delta.ps \\
                                    \NONE & \text{otherwise}
                                  \end{array}
                                \right. \\
      \pid(a) &= a >> \PPBITS
...
In \refr{ExecInstr}, the input of the $\decode$ function is a $\WORD$ $i_{w}$(for instruction word)
...
new configuration after executing the instruction on VM $v$. If $i_{w}$ is an
    ...
    \begin{tabular}[t]{L|L L| L}
      ...
      \instrm{memd}\; r_{1}\; r_{2} &\leftshift \nextt(v, \Delta', m, s')
                                 &\!\!\!\!\! \begin{array}{l}
                                               ...
                                               r =  \Delta[v].rs[r_{2}] \\
                                               s' = s \append [(\pid(pa), \Delta[v].rs[r_{2}])]
                                             \end{array}
                      &\leftshift\begin{array}{l}
