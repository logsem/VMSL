...
% table as the $\PID$ in IP address space is always equal to the corresponding
% one in physical address space. Namely, we always have mappings like
% $x \mapsto x$ in page tables.
...
\refr{RepeatHalt/Fail} rules are applied to proceed the execution. There are two
major reasons on why we didn't use instructions as expressions. On one hand,
