...
\todo{Lars: I don't understand this para} We argue that it is sufficient to record the necessary information on the actual page table.
...
Except claiming explicitly\todo{Lars: ??}, triple $(\Delta, m, s)$ represents an execution
...
consider lists as sets and apply $\in, \cup,$ and $\setminus$ operators on them.\todo{Lars:
  later we should check if/when we rely on the lists being ordered}
...
$\INSTR$ which has a predefined semantic interpretation.\todo{Lars: in that rule, is $m$ the global memory ? It should be a parameter somehow to the rule, I guess}
