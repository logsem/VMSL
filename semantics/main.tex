...
\todo[inline]{Zongyuan: But for
me, what is sopposed to be the $\mu_{j}$ is still unclear. I personally don't
think it can simply be $\mathsf{ExeInstr}_{j}$.}
...
\begin{align*}
  \wpre{(e,i)}[\mathcal{E}]{\Phi} \defined & (\pvs_{\mathcal{E}} \Phi(e)) \lor \\
  &\Big(e \notin \text{Val} \land \forall \sigma . \text{CurVM}(\sigma,i) \wand \mathcal{S}(\sigma) \wand \pvs[\mathcal{E}][\emptyset] \Big(\\
  &\big( \text{red}(e,\sigma) \land \later \forall e', \sigma'. \big((e,\sigma) \tlr (e',\sigma') \wand \\
   &~~~~~\pvs[\emptyset][\mathcal{E}] (\mathcal{S}(\sigma') \asts \wpre{(e',i)}[\mathcal{E}]{\Phi} )\big)\Big)  \\
 % &\big( i \ne j \land \pvs[\emptyset][\mathcal{E}] (\mathcal{S}(\sigma) \asts \Phi(e) )  \big) \Big) \Big)\\
  \mathsf{wps} _{\mathcal{E}} ~\vec{es} ~ \big\{ \Phi s   \big\} \defined & \Sep_{(e,i) \in \vec{es}} \wpre{(e,i)}[\mathcal{E}]{\Phi s(i)}
\end{align*}
% \todo[inline]{Zongyuan: It seems to be problematic to have the big star. The main issue is it is not possible that all VMs go to the terminatal states. In fact, if we think globally, one VM's halting will stop the whole system. At that point, all other VMs are supposed to have mode \textsf{Repeat Done NextInstr}, which is apperently not a $\DONE$.}

% \todo[inline]{Zongyuan: Adequacy??}
...
%   \inferrule[wp-wand]{}{
%    (\forall v. \Phi(v) \wand \Psi(v)) \asts \wpre{\mu}[i]{v.\Phi(v)} \vdash \wpre{\mu}[i]{v.\Psi(v)}
%  }\\

%  %  \inferrule[wp-done]{}{
%  %    \Phi(\mathsf{Done~\theta}) \vdash \wpre{\mathsf{Done~\theta}~|~i}{v.\Phi(v)}
%  % }\\

%   \inferrule[wp-repeat-exec]{}{
%     \wpre{\mathsf{ExecInstr}}[i]{\mu.\wpre{\mathsf{Repeat~}\mu}[i]{v.\Phi(v)}}   \vdash \wpre{\mathsf{Repeat~ExecInstr}}[i]{v.\Phi(v)}
%   }

%   \inferrule[wp-repeat-next]{}{
%     \wpre{\mathsf{Repeat~ExecInstr}}[i]{v.\Phi(v)}   \vdash \wpre{\mathsf{Repeat~Done~NextInstr}}[i]{v.\Phi(v)}
%   }

%   \inferrule[wp-repeat-halt]{}{
%     \Phi(\mathsf{Done~Halt})   \vdash \wpre{\mathsf{Repeat~Done~Halt}}[i]{v.\Phi(v)}
%   }

%   \inferrule[wp-repeat-fail]{}{
%     \Phi(\mathsf{Done~Fail})   \vdash \wpre{\mathsf{Repeat~Done~Fail}}[i]{v.\Phi(v)}
%   }
...
Derivable rules:
\begin{mathpar}
\inferrule[Number-Duplicate]{}{\ownGGhost{\#~n} \wand \ownGGhost{\#~n} \asts \ownGGhost{\#~n}}

\inferrule[Number-Agree]{}{\ownGGhost{\#~n} \asts \ownGGhost{\#~m} \ra m=n}
\end{mathpar}
...
Derivable rules:
\begin{mathpar}
\inferrule[Mem-Split]{}{a \gmapsto_{p+q} w \wand a \gmapsto_{p} w \asts a \gmapsto_{q} w }

\inferrule[Mem-Combine]{p+q \le 1\\ w=v}{a \gmapsto_{p} w \asts a \gmapsto_{q} v \wand a \gmapsto_{p+q} w}
...
Derivable rules:
\begin{mathpar}
\inferrule[Reg-Split]{}{r \regmap{i}_{p+q} w \wand r \regmap{i}_{p} w \asts r \regmap{i}_{q}w }

\inferrule[Reg-Combine]{p+q \le 1\\ w=v}{r \regmap{i}_{p} w \asts r \regmap{i}_{q} v \wand  r \regmap{i}_{p+q} w}
...
\end{align*}
\begin{mathpar}
\inferrule[Tx-Duplicate]{}{\text{TX}^{i}(p) \wand \text{TX}^{i}(p) \asts \text{TX}^{i}(p)  }

\inferrule[Rx-Duplicate]{}{\text{RX}^{i}(p) \wand \text{RX}^{i}(p) \asts \text{RX}^{i}(p)  }

\inferrule[Rx-Split1]{}{\text{RX}^{i}_{q1+q2}(p,b,n,v) \wand \text{RX}^{i}_{q1}(p,b,n,v) \asts \text{RX}^{i}_{q2}(p,b,n,v)  }

\inferrule[Rx-Split2]{}{\text{RX}^{i}_{q}(p,b,n,v) \wand \text{RX}^{i}_{q}(p,b,n,v) \asts \text{RX}^{i}(p)  }
...
% \end{align*}
% \begin{mathpar}
% \inferrule[Owned-Split]{}{\text{Owned}(s1 \uplus s2) \wand \text{Owned}(s1) \asts \text{Owned}(s2)  }
...
\end{align*}
\begin{mathpar}
\inferrule[Owned-Split]{}{\text{Owned}^i_{q_1+q_2}(s1 \uplus s2) \wand \text{Owned}^i_{q_1}(s1) \asts \text{Owned}^i_{q_2}(s2)  }\\
\inferrule[Owned-Update]{}{\text{Owned}^i_1(s) \wand \text{Owned}^i_1(s')}
...
% \end{align*}
% \begin{mathpar}
% \inferrule[Access-Shared-Split]{}{\text{Accessible}_{q1+q2}(p,SA)\wand \text{Accessible}_{q1}(p,SA) \asts \text{Accessible}_{q2}(p,SA)}
% \end{mathpar}
...
\end{align*}
\begin{mathpar}
\inferrule[Access-Shared-Split]{}{\text{Accessible}^i_{q1+q2}(p,SA)\wand \text{Accessible}^i_{q1}(p,SA) \asts \text{Accessible}^i_{q2}(p,SA)}
\end{mathpar}
...
\end{align*}
\begin{mathpar}
\inferrule[Transaction-Split]{}{\text{Transaction}_{q1+q2}(w_{h},v,w_{f},w_{t},pgs,fid)\wand  \text{Transaction}_{q1}(w_{h},v,w_{f},w_{t},pgs,fid) \asts \\  \text{Transaction}_{q2}(w_{h},v,w_{f},w_{t},pgs,fid) }\\

\inferrule[Retrievable-Split]{}{\text{Retrievable}(w_{h},vs1 \uplus vs2)\wand \text{Retrievable}(w_{h},vs1) \asts \text{Retrievable}(w_{h},vs2)}
\end{mathpar}
