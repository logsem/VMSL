...
\begin{align*}
a \gmapsto w& \eqdef \ownGhost{\gname_{MM}}{\circ [a \mapsfrom (1,\mathsf{ag}(w)]}\\
...
\begin{align*}
r \regmap{i} w& \eqdef \ownGhost{\gname_{REG}}{\circ [(r,i) \mapsfrom (1,\mathsf{ag}(w)]}\\
...
\begin{align*}
\text{TX}_{i}(p) &\eqdef \ownGhost{\gname_{TX}}{\circ [i \mapsfrom \mathsf{ag}(p)]}\\
\SI & \eqdef \ownGhost{\gname_{TX}}{\bullet \{[j \mapsfrom \mathsf{ag}(\Phi.\delta s[j].mb.tx)] ~|~ j \in [0,|\Phi.\delta s|)\cap \nat  \}}\\
\text{RX}_{i}(p,b,n,v) &\eqdef \ownGhost{\gname_{RX}}{\circ [i \mapsfrom (\mathsf{ag}(p),(1,\mathsf{ag}(b,n,v))^{?}))]}\\
...
\textbf{Page tables.} The page table of a VM can be regarded as two collections of $\PID$s. One records the ownership of pages, the other one records the accessibility. The first one can be simply modeled as $\text{Auth}(\mathcal{P}(\VMID))$ with disjoint union as the composition operation.
\begin{align*}
\text{Owned}(p) &\eqdef \ownGhost{\gname_{OWN}}{\circ \{ p \}} \\
\SI & \eqdef \ownGhost{\gname_{OWN}}{\bullet \{p ~|~ [p \mapsfrom (\text{O}, -)] \in \Phi.\delta s[i].pt \}}
\end{align*}
The second one needs a Frac to keep track of shared access: $\text{Auth}(\PID \finmap (\text{Frac}, \AG{} +\text{Ex}())$.
\begin{align*}
...
\textbf{Transactions.}
Transactions are indexed by the handle in $\SSS$. Thus, the basic construction is a partial finite map. We then divide transactions into two parts. The first is the ``constant'' part modeled by $\text{Auth}(\WORD \finmap (\text{Frac} \times \AG{\VMID \times \WORD \times \WORD \times (\VMID \finmap [\PID]) \times \FID)}))$. This part won't be changed during the transaction, so that we can simply apply the agreement construction. In order to make freeing handles possible, we additionally add a Frac to the construction. The second part is $\Auth(\WORD \finmap \mathcal{P}(\VMID))$ with pointwise disjoint union as the composition operation.  A element $v$ in the set can be considered as a token for the VM $v$ to retrieve.
\begin{align*}
\text{Transaction}_{q}(v,w_{f},w_{t},w_{h},pgs,fid) &\eqdef \ownGhost{\gname_{TRANS}}{\circ [w_{h}\mapsfrom (q,\textsf{ag}(v,w_{f},w_{t},pgs, fid))]} \\
\SI &\eqdef \ownGhost{\gname_{TRANS}}{\bullet \{[w_{h}\mapsfrom (1,\textsf{ag}(ss.1,ss.2,ss.3,ss.5, ss.6))] ~|~ [w_{h}\mapsfrom ss]\in \Phi.sss \}}\\
\text{Retrievable}(w_{h},v) &\eqdef \ownGhost{\gname_{RETRI}}{\circ [w_{h}\mapsfrom \{v\}]}\\
