...
\begin{figure}
  \begin{mathpar}
    ...
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o\ge w_l}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{INV\_PARA}\end{array}}\right\},mm,sss))}

    \and \inferrule[Exec-hvc-Succ-MemDonate-Error-NotSender]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v=\tv{mm[\addr(pid_t,0)]}}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{DENIED}\end{array}}\right\},mm,sss))}

    \and \inferrule[Exec-hvc-Succ-MemDonate-Error-NotSender]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v\ne \tv{mm[\addr(pid_t,0)]}}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{DENIED}\end{array}}\right\},mm,sss))}

    \and \inferrule[Exec-hvc-Succ-MemDonate-Error-NotOwendExcl]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v=\tv{mm[\addr(pid_t,0)]} \\pid_d = \pid(mm[\addr(pid_t,w_o+1)]) \\ !\text{OwnedExclPage}(\delta_v,pid_d) }
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{DENIED}\end{array}}\right\},mm,sss))}
 \end{mathpar}
\end{figure}
\begin{figure}
  \begin{mathpar}
    ...
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v=\tv{mm[\addr(pid_t,0)]} \\pid_d = \pid(mm[\addr(pid_t,w_o+1)]) \\ \text{OwnedExclPage}(\delta_v,pid_d) \\ n=\tv{mm[\addr(pid_t,w_o)]} \\ !\text{ValidVMID}(\delta s,n) \lor n=v}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{INV\_PARA}\end{array}}\right\},mm,sss))}

    \and \inferrule[Exec-hvc-Succ-MemDonate-Error-InvalidFlag]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v=\tv{mm[\addr(pid_t,0)]} \\ pid_d = \pid(mm[\addr(pid_t,w_o+1)]) \\ \text{OwnedExclPage}(\delta_v,pid_d)  \\ n=\tv{mm[\addr(pid_t,w_o)]} \\ \text{ValidVMID}(\delta s,n) \land n\ne v\\ w_f=mm[\addr(pid_t,1)] \\ w_f[63:1]\ne 0}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{INV\_PARA}\end{array}}\right\},mm,sss))}

    \and \inferrule[Exec-hvc-Succ-MemDonate-Error-InvalidHandle]
    {\text{AccessibleAddr}(\delta_v,a_i) \\\text{DecodeInstr}(mm[a_i],\mathtt{hvc}) \\\text{IsFFA}(\delta_v,\tt{MEM\_DONATE})\\ w_l = rs_v[\reg{R1}]\\w_l>\PPMAX\\pid_t=\delta_v.mb.tx\\ w_o=mm[\addr(pid_t,4)]\\w_o< w_l\\v=\tv{mm[\addr(pid_t,0)]} \\ a_p = \ta{mm[\addr(pid_t,w_o+1)]} \\ \text{OwnedExclAddr}(\delta_v,a_p) \\ n=\tv{mm[\addr(pid_t,w_o)]} \\ \text{ValidVMID}(\delta s,n) \land n\ne v\\ w_f=mm[\addr(pid_t,1)] \\ w_f[63:1]=0 \\ mm[\addr(pid_t,2)] \ne 0}
    {(\EI{v} ,\,(\delta s,mm,sss)) \rightarrow (\DNNXT{v} ,\, (\delta s \text{ with }\left\{{\begin{array}{l}[v].rs[\reg{pc}]=rs_v[\reg{pc}]+1;\\\null[v].rs[\reg{R0}]=\efid{ERROR};\\\null[v].rs[\reg{R2}]=\eec{INV\_PARA}\end{array}}\right\},mm,sss))}
