...
\begin{align*}
  \wpre{\mu~|~i}{v.\Phi(v)} \defined & (\mu \in \DONE \land \upd \Phi(\mu)) \lor \\
  ...
  &~~~~~\upd (\mathcal{S}(\phi') \asts \wpre{\mu'~|~i}{v.\Phi(v)} \asts \Sep_{(\mu'',j) \in \vec{\mu_{f}}} \wpre{\mu''~|~j}{v.\TRUE} )\big) \Big) \Big)
\end{align*}

The last missing piece is an expression for VM $-1$ to start new VMs. Thus, we provide a new execution mode $\mathsf{Start}~l$, where $l$ is a list of VMIDs. The corresponding thread local reduction rule and weakestpre rule are the following:
