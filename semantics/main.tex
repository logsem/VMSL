...
Intuitively, the hypervisor forward the MSG\_WAIT to the scheduler. The scheduler
...
MSG\_SEND plus $w_{s}$ and the receiver VM $v_{r}$. The hypervisor then checkes
...
control to the scheduler($v_{n}=0$). When giving away the control, hypervisor
...
so that the scheduler can schedule accordingly. Otherwise, if primary VM is
sending the message, then it already knew the sender and the recevier.
